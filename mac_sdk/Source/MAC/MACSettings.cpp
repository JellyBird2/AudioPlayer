...
bool CMACSettings::Load()
{
    ...
    int nDefaultSimultaneousFiles = APE_MAX<int>(4, static_cast<int>(std::thread::hardware_concurrency() / 4));
...
CString CMACSettings::GetModeName() const
{
    CString strRetVal;
...
CString CMACSettings::GetCompressionName() const
{
    CString strRetVal;
